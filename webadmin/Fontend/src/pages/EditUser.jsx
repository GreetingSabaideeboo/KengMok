...
function EditUser() {
    ...
    useEffect(() => {
        Axios.post("http://localhost:6957/getUser", {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        Axios.post('http://localhost:6957/updateUser', {
    ...
    const deLete = async (UID) => {
        if (confirm("Delete User " + UID + "?") == true) {
          try {
            const response = await Axios.post('http://localhost:6957/changeStatus', { uid: UID });
            peopleList()
          } catch (error) {
